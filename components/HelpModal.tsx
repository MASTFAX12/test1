    ...
    <div className="w-full h-full bg-slate-100 p-4 rounded-lg flex items-center justify-center">
        <div className="bg-white p-3 rounded-lg shadow-lg relative w-52">
            ...
            <div className="absolute -bottom-4 -left-4 flex items-center gap-1 bg-white p-1 rounded-md border shadow-md">
                ...
                <PencilIcon className="w-6 h-6 p-1 text-green-500 bg-green-50 rounded"/>
...
const New_S5_Visual: FC = () => (
    <div className="w-full h-full bg-slate-100 p-4 rounded-lg flex items-center justify-center gap-4">
        <div className="w-24 bg-white p-2 rounded-lg shadow-md space-y-2">
            ...
            <div className="h-4 bg-blue-500 rounded text-white text-xs text-center">الأرشيف</div>
            <div className="h-4 bg-slate-200 rounded"></div>
        </div>
         <svg width="40" height="20" viewBox="0 0 40 20" className="text-slate-400">
        ...
        <div className="w-48 bg-white p-2 rounded-lg shadow-md space-y-2 border-t-4 border-blue-500">
...
const New_D2_Visual: FC = () => (
     <div className="w-full h-full bg-slate-100 p-4 rounded-lg flex items-center justify-center gap-4 text-xs font-bold text-slate-500">
        ...
        <div className="w-32 text-center">
            <p className="mb-2">بانتظار الفحص</p>
...
const New_D3_Visual: FC = () => (
    <div className="w-full h-full bg-slate-100 p-4 rounded-lg flex flex-col items-center justify-center text-xs font-bold text-slate-500">
        <div className="bg-white p-2 rounded-lg shadow-md border-2 border-blue-400 h-16 w-32 mb-2 flex items-center justify-center relative">
            <p>قيد المعالجة</p>
...
const secretarySteps: HelpStep[] = [
    ...
    { 
        ...
        description: "عند تمرير الفأرة على البطاقة، ستظهر الأزرار. استخدم الجرس (🔔) للنداء، القلم (✏️) للتعديل، والدولار (💲) لتسجيل الدفع. يمكنك أيضاً سحب البطاقة لتغيير ترتيبها.",
        visual: New_S2_Visual
    },
    { 
        title: "الخطوة 3: إرسال المراجع للطبيب", 
        description: "عندما يحين دور المراجع، اضغط على أيقونة الصح (✔). هذا الإجراء ينقله إلى قائمة 'قيد المعالجة'، وهو إشعار للطبيب بأن المراجع جاهز في غرفة الفحص.",
        visual: S3_Visual
    },
    { 
        title: "الخطوة 4: التعامل مع الحالات", 
        description: "إذا غادر مراجع قبل الفحص، استخدم أيقونة سلة المهملات (🗑️) لأرشفته (إلغاء) أو حذفه نهائياً من قائمة اليوم.",
...
const doctorSteps: HelpStep[] = [
    {
        ...
        description: "المرضى الجاهزون للفحص سيظهرون لك مباشرة في قائمة 'قيد المعالجة'. هذه هي قائمة عملك الأساسية.",
        visual: D1_Visual
    },
    {
        title: "الخطوة 2: أثناء الفحص",
        description: "استخدم أيقونة الملاحظات (📋) لتدوين التشخيص والعلاج. إذا كان المراجع بحاجة لفحوصات إضافية، أرسله إلى قائمة 'بانتظار الفحص' باستخدام أيقونة الطائرة الورقية (✈️).",
        visual: New_D2_Visual
    },
    {
        title: "الخطوة 3: إنهاء الزيارة",
        description: "عند الانتهاء، اضغط على أيقونة الصح (✔) لنقل الزيارة إلى الأرشيف. إذا احتجت لإعادة المراجع إلى قائمة الانتظار لأي سبب، استخدم أيقونة الإرجاع (↩️).",
